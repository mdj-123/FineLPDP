...
def predict_defective_files_in_releases(args, dataset_name):
    ...
    checkpoint = torch.load(actual_save_model_dir + 'epoch-' + epoch + '-model.pth')
    ...
    for rel in test_rel:
        print('V6 within: generating prediction of {} via {}'.format(rel, dataset_name))
        ...
        for filename, df in tqdm(test_df.groupby('filename')):
            ...
            with torch.no_grad():
                ...
                output, line_scores, _ , file_feature = model([cov_input])
                line_scores = sig(line_scores)
                file_prob = sig(output).item()
                prediction = bool(round(file_prob))
                all_file_labels.append(file_label)
                all_file_probs.append(file_prob)

            torch.cuda.empty_cache()

            numpy_line_attn = line_scores[0].cpu().detach().numpy()
            ...
            for i in range(0, len(code)):
                ...
                cur_line_attn = float(numpy_line_attn[i])
        ...
        file_auc = roc_auc_score(all_file_labels, all_file_probs)
        print(f'[V6 within {dataset_name}] File-level AUC: {file_auc:.4f} epoch: {epoch}')
...
def main():
    ...
    arg.add_argument('-save_model_dir', type=str, default='output/model/FineLPDP/',
                     help='the save directory of model')
    arg.add_argument('-prediction_dir', type=str, default='output/prediction/FineLPDP/within-release/',
    ...
    arg.add_argument('-target', type=int, default=7, help='target epoch')
    ...
    arg.add_argument('-model_name_or_path', type=str, default='./codebert-base',
                     help='the model checkpoint for weights initialization')
    arg.add_argument('-config_name', type=str, default=None,
                     help='optional pretrained config name or path if not the same as model_name_or_path')
    arg.add_argument('-tokenizer_name', type=str, default='./codebert-base',
